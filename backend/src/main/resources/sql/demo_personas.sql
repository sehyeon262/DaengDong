...
-- 경로 선택 로그 (개인화 학습 데이터 - 최근 30일)
-- 대전 유성구 카페/식당 위주로 선택한 이력
...
-- 선택 로그에 연결된 장소 (카페/식당 위주)
-- 대전 유성구 카페: 712(강아지왈츠카페), 659(강아지공작소→미용이지만 근처)
-- 실제 place_id는 DB에 있는 대전 카페/식당 ID 사용
INSERT INTO route_selection_places (selection_log_id, place_id, visit_order)
SELECT sl.id, p.id, ROW_NUMBER() OVER (PARTITION BY sl.id ORDER BY p.id)
FROM route_selection_logs sl
CROSS JOIN LATERAL (
    SELECT id FROM places
    WHERE category_id IN (6, 7)  -- 카페, 식당
      AND address LIKE '%대전%'
    ORDER BY RANDOM()
    LIMIT 3
) p
...
-- 선택 로그에 연결된 장소 (여행지/박물관/미술관 위주)
INSERT INTO route_selection_places (selection_log_id, place_id, visit_order)
SELECT sl.id, p.id, ROW_NUMBER() OVER (PARTITION BY sl.id ORDER BY p.id)
FROM route_selection_logs sl
CROSS JOIN LATERAL (
    SELECT id FROM places
    WHERE category_id IN (11, 8, 9)  -- 여행지, 박물관, 미술관
      AND address LIKE '%대전%'
    ORDER BY RANDOM()
    LIMIT 3
) p
...
-- =============================================
-- 확인 쿼리: 페르소나별 개인화 상태 확인
-- =============================================
-- SELECT '페르소나 A (김지훈)' AS persona, pc.name AS category, ucp.selection_count, ucp.preference_score
-- FROM user_category_preferences ucp
-- JOIN place_category pc ON pc.id = ucp.category_id
-- WHERE ucp.member_id = 1
-- ORDER BY ucp.preference_score DESC;

-- SELECT '페르소나 B (박민재)' AS persona, pc.name AS category, ucp.selection_count, ucp.preference_score
-- FROM user_category_preferences ucp
-- JOIN place_category pc ON pc.id = ucp.category_id
-- WHERE ucp.member_id = 3
-- ORDER BY ucp.preference_score DESC;
