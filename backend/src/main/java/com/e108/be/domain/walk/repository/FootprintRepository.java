...
public interface FootprintRepository extends JpaRepository<Footprint, Long> {
    ...
    // 발자국 도장 취소 (강아지 ID + 장소 ID 기준 — 모든 산책 도장 삭제)
    @Modifying
    @Query("DELETE FROM Footprint f WHERE f.dogId = :dogId AND f.placeId = :placeId")
    void deleteByDogIdAndPlaceId(@Param("dogId") Long dogId, @Param("placeId") Long placeId);
