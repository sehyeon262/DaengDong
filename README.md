...
> 빅데이터 기반 반려견 맞춤 산책 추천 플랫폼

<p align="center">
  <img src="docs/images/service_cover.png" alt="댕동여지도 서비스 표지" width="800"/>
</p>
...
108명의 반려견 보호자를 대상으로 설문조사를 실시한 결과, 산책 시 가장 큰 불편함은 다음과 같았습니다:
...
보호자들이 원했던 것은 단순합니다: **반려견과 마주치지 않고 안전하게 걷는 시간.** 댕동여지도는 이 네 가지 핵심 문제를 해결합니다:

- 새로운 코스 → **맞춤형 경로 추천**
- 예기치 못한 충돌 → **실시간 위험 알림**
- 안전한 산책 환경 확인 → **날씨·대기질 기반 산책 적합도 판단**
- 매번 같은 산책 → **빅데이터 기반 경로 학습으로 점점 나아지는 추천**
...
| **AI 경로 추천** | 반려견 체형·행동 패턴·환경 데이터 기반 맞춤 산책 코스 3개를 추천합니다 |
...
| **AI 산책 일기** | Google Vision(환경 분석) → ONNX(감정 분석) → LLM(일기 생성) 파이프라인으로 자동 생성합니다 |
| **반려견 감정 분석** | ONNX 모델이 산책 사진에서 반려견의 표정과 감정을 추론합니다 |
...
<p align="center">
  <img src="docs/images/architecture.png" alt="시스템 아키텍처" width="900"/>
</p>

Android 앱에서 HTTPS 요청을 보내면 AWS EC2의 Nginx 리버스 프록시가 `/api`(Prod)와 `/dev`(Dev) 경로로 라우팅합니다. 각 환경은 독립된 Spring Boot, Redis, PostGIS 인스턴스로 구성되며, 외부 서비스(S3, GMS AI, Google Vision, TMap)와 연동합니다. Jenkins가 GitLab Webhook을 받아 브랜치별 자동 빌드/배포를 수행합니다.
...
<p align="center">
  <img src="docs/images/team.png" alt="팀원 소개" width="800"/>
</p>
...
> **너와 나의 연결꼬리팀** — Samsung Software Academy for Youth(SSAFY) 자율 프로젝트
